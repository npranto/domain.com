...
export default function CookieNotice({ onClose }) {
  return (
    <div
      className={`${styles.container} bg-white px-6 py-4 fixed bottom-3 left-3 rounded-md w-full max-w-screen-sm shadow-sm`}
    >
      <button
        type="button"
      ...
      <button
        type="button"
        className="p-3 w-full text-center bg-blue-500 hover:bg-blue-600 text-white mb-2 transition-colors"
        onClick={() => onClose()}
      >
        Got It
      </button>
      <button
        type="button"
        className="p-3 w-full text-center underline text-blue-600"
        onClick={() => onClose()}
      >
        Cookie Settings
      </button>
